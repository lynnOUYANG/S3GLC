...
class Encoder(torch.nn.Module):
    def __init__(self, num_features, dim, num_gc_layers,device,use_weight=False):
    ...
    def forward(self, x, edge_index, batch,edge_attr=None):
        ...
        for i in range(self.num_gc_layers):
            ...
            xs.append(x)
        # for i in range(self.num_gc_layers):
        #     if edge_attr is not None and self.use_weight:
        #         if edge_attr.dim() == 1:
        #             edge_attr = edge_attr.unsqueeze(1)  
        #         x = F.leaky_relu(self.convs[i](x, edge_index, edge_weight=edge_attr))
        #     else:
        #         x = F.leaky_relu(self.convs[i](x, edge_index))
        #     x = self.bns[i](x)
        #     xs.append(x)
