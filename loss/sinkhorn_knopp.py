...
class SemiCurrSinkhornKnopp(torch.nn.Module):
    ...
    def forward(self, P):
        ...
        P = P.detach().double()
        device = P.device  # Get the device from input tensor
        ...
        # prior distribution
        Pa = torch.ones(n, 1, device=device) / n  # how many samples
        Pb = self.rho * torch.ones(Q.shape[1], 1, device=device) / k # how many prototypes
        Pb[-1] = 1 - self.rho

        # init b
        b = torch.ones(Q.shape[1], 1, device=device, dtype=torch.double) / Q.shape[1] if self.b is None else self.b
...
if __name__=="__main__":
    ...
    logits = torch.rand(100, 4, device=device)
