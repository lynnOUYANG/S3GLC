...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
        ...
        if not (self.name == 'MUTAG' or self.name == 'PTC_MR' or self.name == 'DD' or self.name == 'PROTEINS' or self.name == 'NCI1' or self.name == 'NCI109' or self.name == 'ER_MD'):
    ...
    def get(self, idx):
        ...
        # Handle case where edge_index might be None or empty (graph with no edges)
        if data.edge_index is None or data.edge_index.numel() == 0:
            # Determine node_num from data.x or num_nodes
            if data.x is not None:
                node_num = data.x.size(0)
            elif hasattr(data, 'num_nodes') and data.num_nodes is not None:
                node_num = data.num_nodes
            else:
                # Fallback: create a single-node graph
                node_num = 1
            # Create an empty edge_index tensor if it's None
            if data.edge_index is None:
                data.edge_index = torch.empty((2, 0), dtype=torch.long)
        else:
            node_num = data.edge_index.max().item() + 1
        
        # Only add self-loops if there are nodes
        if node_num > 0:
        ...
        if self.aug == 'dnodes':
            ...
            # Use node_num computed above, or determine from edge_index if available
            if data_aug.edge_index is not None and data_aug.edge_index.numel() > 0:
                aug_node_num = data_aug.edge_index.max().item() + 1
            elif data_aug.x is not None:
                aug_node_num = data_aug.x.size(0)
            else:
                aug_node_num = node_num  # Use the node_num computed earlier
            data_aug.x = torch.ones((aug_node_num, 1))
